...
factorsContainingV = unique([G.var2factors{V}]);

factorsNeeded = repmat(struct('var', [], 'card', [], 'val', []), 1, length(factorsContainingV));

factorsNeeded = F(factorsContainingV);
    
for i = 1:length(LogBS)    
    %ComputeProbability = P(V = i | Other variables = current value)
    A(V) = i;
    LogBS(i) = LogProbOfJointAssignment(factorsNeeded, A);    
end
