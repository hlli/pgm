...
for i = 1:length(G.names)
    ...
    %
    % Also, note that randsample() requires arguments in raw probability
    % space
    % be sure that the arguments you pass to it meet that criteria
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    rawDistribution = exp(BlockLogDistribution(i, G, F, A));
    
    A(i) = randsample(1:G.card(i), 1, true, rawDistribution);
