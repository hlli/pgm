...
logCurrentPi = LogProbOfJointAssignment(F, A);
logNewPi = LogProbOfJointAssignment(F, A_prop);

prob = exp(logNewPi - logCurrentPi + log_QY_ratio + LogR(old_value) - LogR(new_value));

p_acceptance = min(1, prob);
